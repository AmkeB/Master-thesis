...
def microscope_FACS_sorting(filepath, i, file_WL, file_R, tagging_cells, min_A=1000, max_A=10000, thres_low=1000, thres_high=3000, bnd_ctoff=1/5.12):
    ...
    N_fluor = np.sum(fluor_I > thres_high)
    N_WL = np.sum(fluor_I < thres_low)
    N_unsure = len(fluor_I) - N_fluor - N_WL
    print("{:04d} is fluor,\t{:04d} is WL,\t{:04d} is unsure".format(
        N_fluor, N_WL, N_unsure))
    ...
    # return blobs_new, yxr_NLS, xyr_NLS, yxr_mScar, xyr_mScar, yxr_unsure, xyr_unsure
...
def shift_image2(img, dy_pix, dx_pix):
    #this one uses zero-padding when shifting image instead of filling the empty part with max value
    img_after = deepcopy(img)
...
def load_images_and_find_shift(file_path, i, file1, file2, coord, dz, endsize=81, max_dist=10, min_shift=30,downsize_ufo2_img = False):
    """
    ...
    if downsize_ufo2_img == True:
        parts_size = len(img2)
        img1 = cv2.resize(img1,(parts_size,parts_size))
        print("it downsized ufo2 image")
    
    if downsize_ufo2_img == False:
    ...
    if downsize_ufo2_img == True:
        fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(20, 20))
        ax[0, 0].imshow(auto_blur_contrast(img1[500:1000, 500:1000]))
        # ax[0,0].set_title("0,0")
        ax[1, 0].imshow(auto_blur_contrast(img2[500:1000, 500:1000]))
        # ax[1,0].set_title("1,0")
        ax[0, 1].imshow(auto_blur_contrast(img1[500:1000, 500:1000]))
        # ax[0,1].set_title("0,1")
        ax[1, 1].imshow(auto_blur_contrast(img2_after[500:1000, 500:1000]))
        # ax[1,1].set_title("1,1")
        fig.savefig(file_path+"\\shift plots\\ROI{:03d}.png".format(i))
...
def find_best_rotation(file1, file2, rotations):
    #img1: should be ufo2 image
    #img2: should be parts image

    # load and process ufo2 image
    img1 = cv2.imread(file1, cv2.IMREAD_GRAYSCALE)
    ...
    img1 = auto_blur_contrast(img1)
    # img1 = tophat(img1)
    
    # proces parts image
    img2 = auto_blur_contrast(img2)
    img2 = cv2.convertScaleAbs(img2, alpha=(255/65535))
    img2 = tophat(img2)

    # initialize arrays for storage
    shifts_x = np.zeros(len(rotations))
    shifts_y = np.zeros(len(rotations))
    shifts_x_plus_y = np.zeros(len(rotations))
    errors = np.zeros(len(rotations))
    max_convs = np.zeros(len(rotations))
    
    # for each rotation, find the one that gives best result in phase-cross correlation
    for i in range(len(rotations)):
        rot = rotations[i]
        img_ufo_rotated = imutils.rotate_bound(img1, rot)
        
        #resize such that it gets back the 1164x1164 size
        middle = int(len(img_ufo_rotated)/2)
        half_size = int(parts_size/2)
        img_ufo_resized = img_ufo_rotated[(middle-half_size):(middle+half_size),(middle-half_size):(middle+half_size)]
        img_ufo_rotated = img_ufo_resized
        # all_ufo_images.append(img_ufo_rotated)
        img_ufo_t = tophat(img_ufo_rotated)
        # plt.imshow(img_ufo_t)
        
        shift,error,phasediff = phase_cross_correlation(img_ufo_t,img2,normalization='phase')
        print(shift)
        print(error)
        print(phasediff)
        shifts_x[i] = shift[1]
        shifts_y[i] = shift[0]
        shifts_x_plus_y[i] = shift[0]+shift[1]
        errors[i] = error
        
        shifted_img = shift_image(img_ufo_t,int(-shift[0]),int(-shift[1]))
        # plt.imshow(shifted_img)
        # plt.title('ufo2 shifted')
        # plt.show()
        
        if (abs(shift[0])+abs(shift[1])) < 200:
            conv = fftconvolve(img2,
                               shifted_img[::-1, ::-1])
    
            max_convs[i] = np.max(conv)
            print('max conv',np.max(conv))

    
    best_rot_index = np.where(max_convs == np.max(max_convs))
    print(best_rot_index)
    best_rotation = rotations[int(best_rot_index[0])]
    print('best rotation',best_rotation)
    print('dx',shifts_x[int(best_rot_index[0])])
    print('dy',shifts_y[int(best_rot_index[0])])
    
    return best_rotation


def load_images_and_find_shift_for_PARTS_v3(file_path, i, file1, file2, coord, dz, rotations, include_rot = True, endsize=81, max_dist=10, min_shift=30,downsize_ufo2_img = False):
    """
    ...
    # load and process ufo2 image
    img1 = cv2.imread(file1, cv2.IMREAD_GRAYSCALE)
    img1_original = cv2.imread(file1,cv2.IMREAD_UNCHANGED)
    img2 = cv2.imread(file2, cv2.IMREAD_UNCHANGED)
    ...
    elif include_rot == False:
        rotations_used = [0]
        
        
    #find shift for all given rotations    
    shifts_x = np.zeros(len(rotations_used))
    shifts_y = np.zeros(len(rotations_used))
    shifts_x_plus_y = np.zeros(len(rotations_used))
    errors = np.zeros(len(rotations_used))
    max_convs = np.zeros(len(rotations_used))
    # shifted_ufo_imgs = []
    all_shifts = []
    
    for r in range(len(rotations_used)):
        rot = rotations_used[r]
        img_ufo = deepcopy(img1)
        img_ufo_rotated = imutils.rotate_bound(img_ufo, rot)
        ...
        shift,error,phasediff = phase_cross_correlation(img_ufo_t,img2,normalization='phase') #'phase'
        shifts_x[r] = shift[1]
        shifts_y[r] = shift[0]
        shifts_x_plus_y[r] = shift[0]+shift[1]
        errors[r] = error
        
        shifted_ufo_img = shift_image(img_ufo_t,int(-shift[0]),int(-shift[1]))
        all_shifts.append(shift)

        
        # calculate convolution to find which overlap was best
        if (abs(shift[0])+abs(shift[1])) < 200:
            conv = fftconvolve(img2,
                               shifted_ufo_img[::-1, ::-1])
    
            max_convs[r] = np.max(conv)
            # print('max conv',np.max(conv))
    ...
    # print('BEST_ROT_INDEX',best_rot_index)
    if len(best_rot_index[0])>1:
        best_rot_index = [best_rot_index[0][0]] #quick fix for now if multiple best rotations are found: just take the first
    
    best_rotation = rotations_used[int(best_rot_index[0])]
    print('best rotation',best_rotation)
    print('dx',shifts_x[int(best_rot_index[0])])
    print('dy',shifts_y[int(best_rot_index[0])])
    
    #get the final shifted ufo2 image to store
    shift = all_shifts[int(best_rot_index[0])]
    
    img_final_rotated = imutils.rotate_bound(img1_original, best_rotation)
    
    #resize such that it gets back the 1164x1164 size
    middle = int(len(img_final_rotated)/2)
    half_size = int(parts_size/2)
    img_final_ufo_resized = img_final_rotated[(middle-half_size):(middle+half_size),(middle-half_size):(middle+half_size)]
    img_ufo_final_rotated = img_final_ufo_resized
    
    final_shifted_ufo_img = shift_image(img_ufo_final_rotated,int(-shift[0]),int(-shift[1]))
    
    cv2.imwrite(file_path+"\\ufo2_shifted_imgs_v3\\ROI%03d.tif"%i,final_shifted_ufo_img)
    np.savetxt(file_path+"\\best_rotations_v3\\ROI%03d.txt"%i,[best_rotation],delimiter='\t')
    np.savetxt(file_path+"\\best_shifts_v3\\ROI%03d.txt"%i,shift,delimiter='\t')
